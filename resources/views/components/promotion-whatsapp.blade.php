...
@php
    ...
    $subtitle = $isPremium
        ? __('Maximum attention and standout badge')
        : __('Rise to the top of search results');

    $safeTitle = addslashes($title);
    $siteName = config('app.full_name', 'KibrisKare.com');
@endphp
...
<div x-data="{
    ...
    <div style="margin-bottom:16px;">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;">
            ...
            <button type="button"
                ...
                @if($day === 3)
                <div style="position:absolute;top:-9px;left:50%;transform:translateX(-50%);background:{{ $accent }};color:#fff;font-size:8px;font-weight:900;text-transform:uppercase;letter-spacing:.06em;padding:3px 9px;border-radius:999px;white-space:nowrap;">{{ __('Popular') }}</div>
                @endif
                <div style="font-size:13px;font-weight:700;color:#111827;">{{ $day }} {{ __('times') }}</div>
    ...
    <a @click.prevent="send()" :href="'https://wa.me/{{ $cleanWa }}?text=' + encodeURIComponent('Salam, {{ $siteName }} saytındakı #{{ $id }} nömrəli {{ $itemLabel }}nı (\'{{ $safeTitle }}\') ' + selected + ' DƏFƏ {{ $verb }} istəyirəm (' + prices[selected] + ').')"
       ...
       style="display:flex;align-items:center;justify-content:center;gap:9px;width:100%;background:#25D366;color:#fff;font-size:13px;font-weight:700;padding:12px 16px;border-radius:12px;text-decoration:none;box-sizing:border-box;">
        ...
        <span>{{ __('Order via WhatsApp') }} (<span x-text="prices[selected]"></span>)</span>
    </a>

    <p style="margin:12px 0 0;text-align:center;font-size:11px;color:#9ca3af;line-height:1.4;">
        {{ __('After your order is confirmed, your listing will be activated as premium/boosted.') }}
    </p>
</div>
