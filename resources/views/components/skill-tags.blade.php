...
<div {{ $attributes->merge(['class' => 'flex flex-wrap gap-1.5']) }}>
    @foreach(array_slice($skillsList, 0, $limit) as $skill)
        <span class="px-2 py-0.5 bg-gray-50 text-gray-600 rounded text-[12px] border border-gray-100 font-medium font-mono">{{ $skill }}</span>
    @endforeach
    @if(count($skillsList) > $limit)
        <span class="px-1.5 py-0.5 text-gray-400 text-[11px] font-mono">+{{ count($skillsList) - $limit }}</span>
    @endif
</div>
@endif
