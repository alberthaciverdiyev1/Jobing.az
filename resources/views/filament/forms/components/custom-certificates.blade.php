<div x-data="{
    ...
    <template x-for="(item, index) in state" :key="index">
        <div class="p-4 rounded-xl bg-gray-50/70 dark:bg-gray-800/50 space-y-3">
            <div class="flex items-center justify-between border-b border-gray-200/60 dark:border-gray-700/60 pb-2">
                <span class="text-xs font-semibold text-orange-600 dark:text-orange-400" x-text="(index + 1) + '. Sertifikat' + (item.name ? ' (' + item.name + ')' : '')"></span>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
                <div class="space-y-1">
                    <label class="font-semibold text-gray-700 dark:text-gray-300 block">{{ __('Certificate Name *') }}</label>
                ...
                <div class="space-y-1">
                    <label class="font-semibold text-gray-700 dark:text-gray-300 block">{{ __('Issuing Organization') }}</label>
                    <x-filament::input.wrapper>
                ...
                <div class="space-y-1">
                    <label class="font-semibold text-gray-700 dark:text-gray-300 block">{{ __('Date Received') }}</label>
                    <x-filament::input.wrapper>
                ...
                <div class="space-y-1">
                    <label class="font-semibold text-gray-700 dark:text-gray-300 block">{{ __('Verification URL') }}</label>
                    <x-filament::input.wrapper>
    ...
    <div x-show="!state || state.length === 0" class="text-center py-4 rounded-xl bg-gray-50/50 dark:bg-gray-800/20">
        <p class="text-xs font-medium text-gray-400">{{ __('No certificate added') }}</p>
    </div>
</div>
