<div x-data="{
    ...
    <div class="flex items-center justify-between">
        <span class="text-xs font-semibold text-gray-500 dark:text-gray-400">{{ __('Projects List') }}</span>
    ...
    <template x-for="(item, index) in state" :key="index">
        <div class="p-4 rounded-xl bg-gray-50/70 dark:bg-gray-800/50 space-y-4">
            <div class="flex items-center justify-between border-b border-gray-200/60 dark:border-gray-700/60 pb-2.5">
                <span class="text-xs font-semibold text-orange-600 dark:text-orange-400" x-text="(index + 1) + '. Layihə' + (item.name ? ' (' + item.name + ')' : '')"></span>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                <div class="space-y-1.5">
                    <label class="font-semibold text-gray-700 dark:text-gray-300 block">{{ __('Project Name *') }}</label>
                    <x-filament::input.wrapper>
                        <x-filament::input type="text" x-model="item.name" placeholder="Örn: kariyer.kibriskare.com Portal" />
                    </x-filament::input.wrapper>
                </div>

                <div class="space-y-1.5">
                    <label class="font-semibold text-gray-700 dark:text-gray-300 block">{{ __('Your Role') }}</label>
                    <x-filament::input.wrapper>
                ...
                <div class="space-y-1.5 md:col-span-2">
                    <label class="font-semibold text-gray-700 dark:text-gray-300 block">{{ __('Technologies') }}</label>
                    <x-filament::input.wrapper>
    ...
    <div x-show="!state || state.length === 0" class="text-center py-5 rounded-xl bg-gray-50/50 dark:bg-gray-800/20">
        <p class="text-xs font-medium text-gray-400">{{ __('No project added') }}</p>
    </div>
</div>
