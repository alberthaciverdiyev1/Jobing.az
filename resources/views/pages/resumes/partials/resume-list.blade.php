...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
    ...
    <div class="bg-white rounded-2xl p-5 border border-gray-200 hover:border-orange-300 hover:shadow-md transition-all duration-200 group flex flex-col justify-between relative cursor-pointer">
        ...
        <div>
            <div class="flex items-start gap-3.5">
                ...
                <div class="min-w-0 flex-1">
                    <h3 class="text-base font-semibold text-gray-900 group-hover:text-primary transition leading-tight truncate">
            ...
            <div class="flex flex-wrap items-center gap-1.5 mt-3.5">
                @if(!empty($resume->skills) && is_array($resume->skills))
                    ...
                    @endphp
                    @if($skTitle)
                    <span class="px-2 py-0.5 rounded-md bg-gray-100 text-gray-700 text-[11px] font-medium">
                        {{ $skTitle }}
                    </span>
                    @endif
                    @endforeach
                    @if(count($resume->skills) > 3)
                    <span class="text-[11px] text-gray-400 font-mono">+{{ count($resume->skills) - 3 }}</span>
                    @endif
                @endif
            </div>
        </div>
        ...
        <div class="pt-3 mt-3 border-t border-gray-100 flex items-center justify-between text-xs">
            ...
            <span class="text-primary group-hover:text-primary-dark font-semibold flex items-center gap-1 transition text-xs">
...
<div class="text-center py-16 bg-white rounded-xl border border-gray-200 p-8 shadow-2xs">
    ...
    <h3 class="text-base font-semibold text-gray-900 mb-1">{{ __('No resumes matching your search') }}</h3>
