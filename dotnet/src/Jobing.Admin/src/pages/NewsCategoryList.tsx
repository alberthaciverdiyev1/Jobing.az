...
export default function NewsCategoryList() {
  ...
  const fetchCategories = async () => {
    ...
    setError('')
    try {
      const res = await api.get(`/news-categories?page=${page}&pageSize=${pageSize}`)
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Xəbər kateqoriyaları</h1>
          <p className="text-gray-500 text-sm mt-1">Kateqoriyaların idarəsi</p>
        </div>
        <button onClick={() => navigate('/news-categories/create')}
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-100">
                  ...
                  <th className="text-center text-xs font-medium text-gray-500 uppercase tracking-wider px-4 py-3">Xəbərlər</th>
              ...
              <tbody className="divide-y divide-gray-50">
                {categories.map((cat) => (
                  <tr key={cat.id} className="hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="px-4 py-4 text-center text-sm text-gray-500">{cat.newsCount}</td>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center justify-end gap-1">
                        <button onClick={() => navigate(`/news-categories/edit/${cat.id}`)}
