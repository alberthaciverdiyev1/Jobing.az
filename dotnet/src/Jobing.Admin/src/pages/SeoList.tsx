...
export default function SeoList() {
  ...
  useEffect(() => { fetchSeo() }, [page])

  const fetchSeo = async () => {
    setLoading(true)
    setError('')
    try {
      const res = await api.get(`/seo?page=${page}&pageSize=${pageSize}`)
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Səhifələrin SEO meta məlumatlarının idarəsi</p>
        </div>
        <button onClick={() => navigate('/seo/create')}
          className="flex items-center gap-2 px-4 py-2 bg-indigo-600 text-white rounded-lg text-sm font-medium hover:bg-indigo-700 transition-colors">
          <Plus className="w-4 h-4" /> Yeni SEO
        </button>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        {loading ? (
          ...
          <div className="p-12 text-center"><p className="text-gray-500">Heç bir SEO qeydi tapılmadı</p></div>
        ) : (
          <>
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-100">
                  <th className="text-left text-xs font-medium text-gray-500 uppercase tracking-wider px-6 py-3">Səhifə</th>
                  <th className="text-left text-xs font-medium text-gray-500 uppercase tracking-wider px-4 py-3">Başlıq</th>
                ...
                {items.map((item) => (
                  <tr key={item.id} className="hover:bg-gray-50/50 transition-colors">
                    <td className="px-6 py-4">
                      <div className="text-sm font-mono font-medium text-gray-900">{item.pageKey}</div>
                      {item.ogImage && (
                        <div className="text-xs text-gray-400 truncate max-w-xs mt-0.5">{item.ogImage}</div>
                      )}
                    </td>
                    <td className="px-4 py-4 text-sm text-gray-500 truncate max-w-md">{item.title?.az || '—'}</td>
