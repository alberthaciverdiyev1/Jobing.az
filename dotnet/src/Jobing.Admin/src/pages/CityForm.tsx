...
export default function CityForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEdit) {
        await api.put(`/cities/${id}`, { name, sortOrder, isActive })
      } else {
        await api.post('/cities', { name, sortOrder })
      }
      navigate('/cities')
    } catch (err: any) {
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <div className="flex items-center gap-4">
        <button onClick={() => navigate('/cities')} className="p-2 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-lg transition-colors">
          <ArrowLeft className="w-5 h-5" />
        </button>
        <div>
          <h1 className="text-2xl font-bold text-gray-900">{isEdit ? 'Şəhəri düzəlt' : 'Yeni şəhər'}</h1>
          <p className="text-gray-500 text-sm mt-1">{isEdit ? 'Şəhər məlumatlarını yeniləyin' : 'Yeni şəhər əlavə edin'}</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-gray-200 p-6 space-y-5">
        {LANGUAGES.map(({ key, label }) => (
          <div key={key}>
            ...
            <input
              value={name[key] || ''}
              onChange={e => setName(p => ({ ...p, [key]: e.target.value }))}
