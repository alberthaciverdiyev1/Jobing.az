...
export default function FilterList() {
  ...
  useEffect(() => { fetchFilters() }, [page])

  const fetchFilters = async () => {
    setLoading(true)
    setError('')
    try {
      const res = await api.get(`/filters?page=${page}&pageSize=${pageSize}&sortBy=sortOrder&sortDir=asc`)
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      setMessage('Filter silindi')
      if (filters.length === 1 && page > 1) setPage(p => p - 1)
      else fetchFilters()
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Filterlər</h1>
          <p className="text-gray-500 text-sm mt-1">Filterlərin idarəsi</p>
        </div>
        <button
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        {loading ? (
          ...
          <div className="p-12 text-center">
            <p className="text-gray-500">Heç bir filter tapılmadı</p>
          </div>
        ) : (
          <>
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-100">
                  ...
                  <th className="text-left text-xs font-medium text-gray-500 uppercase tracking-wider px-6 py-3">Açar</th>
                  <th className="text-center text-xs font-medium text-gray-500 uppercase tracking-wider px-4 py-3">Seçimlər</th>
              ...
              <tbody className="divide-y divide-gray-50">
                {filters.map((f) => (
                  <tr key={f.id} className="hover:bg-gray-50/50 transition-colors">
                    <td className="px-6 py-4 text-sm font-medium text-gray-900">{f.name?.az || f.key}</td>
                    <td className="px-6 py-4 text-sm text-gray-500 font-mono">{f.key}</td>
                    ...
                    <td className="px-4 py-4 text-sm text-gray-500 text-center">{f.sortOrder}</td>
                    <td className="px-6 py-4">
                      <div className="flex items-center justify-end gap-1">
                        <button
                          onClick={() => navigate(`/filters/edit/${f.id}`)}
