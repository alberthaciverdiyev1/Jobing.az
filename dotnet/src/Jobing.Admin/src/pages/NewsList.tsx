...
export default function NewsList() {
  ...
  const fetchNews = async () => {
    ...
    setError('')
    try {
      const res = await api.get(`/news?page=${page}&pageSize=${pageSize}`)
  ...
  const handleDelete = async (id: string) => {
    if (!window.confirm('Bu xəbəri silmək istədiyinizə əminsiniz?')) return
    try {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Xəbərlər</h1>
          <p className="text-gray-500 text-sm mt-1">Xəbərlərin idarəsi</p>
        </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        {loading ? (
          ...
          <div className="p-12 text-center"><p className="text-gray-500">Heç bir xəbər tapılmadı</p></div>
        ) : (
          <>
                  ...
                  <tr key={item.id} className="hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="px-4 py-4 text-center">
                      <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${
                        ...
                        {item.isPublished ? 'Yayımlandı' : 'Qaralama'}
                      </span>
                    </td>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center justify-end gap-1">
                        <button onClick={() => navigate(`/news/edit/${item.id}`)}
