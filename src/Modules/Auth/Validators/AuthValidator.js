...
export const registerValidator = (req, res, next) => {
    ...
    if (!confirmPassword) return res.status(400).json({ error: "Şifrə təkrarı tələb olunur" });
    if (!validator.equals(password, confirmPassword)) return res.status(400).json({ error: "Şifrələr uyğun gəlmir" });
