...
export function createJobCard(element, compact = false) {
    ...
    if (element.companyImageUrl && element.companyImageUrl !== "/nologo.png") {
        if (element.companyImageUrl.startsWith('http')) {
            ...
            logoUrl = element.companyImageUrl;
        } else if (element.companyImageUrl.includes('Public/')) {
            logoUrl = '../' + element.companyImageUrl.slice(element.companyImageUrl.indexOf('Public/') + 11);
        } else {
    ...
    if (compact) {
        ...
        return `<div class="job-card group bg-white rounded-xl p-5 cursor-pointer transition-all duration-200 hover:shadow-lg hover:-translate-y-1 border border-gray-300" role="button" tabindex="0" aria-label="${title} - ${company}" data-original-link="${detailLink}">
    ...
    return `<div class="job-card group bg-white rounded-xl p-6 cursor-pointer transition-all duration-200 hover:shadow-lg hover:-translate-y-1 border border-gray-300" role="button" tabindex="0" aria-label="${title} - ${company}" data-original-link="${detailLink}">
